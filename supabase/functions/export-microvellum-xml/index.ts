...
serve(async (req) => {
  ...
  try {
    ...
    // --- Auth gate (review #2) -------------------------------------------
    // This function uses the service-role key (bypasses RLS), so it MUST
    // verify the caller itself: a valid JWT that either owns the job or is an
    // admin. Without this any anon caller could export any job by id.
    ...
    // Owner-or-admin check before returning any job data.
    const [{ data: ownerRow }, { data: isAdmin }] = await Promise.all([
      supabase.from('jobs').select('customer_id').eq('id', jobId).single(),
      supabase.rpc('has_role', { _user_id: user.id, _role: 'admin' }),
    ]);
    const isOwner = ownerRow?.customer_id && ownerRow.customer_id === user.id;
    if (!isOwner && !isAdmin) {
