...
exports.RULE_INDEX = exports.RESERVED_RULE_IDS = exports.RULES = void 0;
exports.evaluateRules = evaluateRules;
exports.ruleWhy = ruleWhy;
...
const GAS_MAX = 600;
function finding(ruleId, tier, message, itemIds) {
...
exports.RULES = [
    {
        ...
        evaluate: ({ design, room }) => design.items.flatMap(item => {
            const r = (0, geometry_1.itemRect)(item);
        ...
        evaluate: ({ floorItems }) => {
            ...
            for (let i = 0; i < floorItems.length; i++) {
                for (let j = i + 1; j < floorItems.length; j++) {
                    if ((0, geometry_1.rectsOverlap)((0, geometry_1.itemRect)(floorItems[i]), (0, geometry_1.itemRect)(floorItems[j]))) {
                        out.push(finding('overlap', 'hard', `${floorItems[i].definitionId} overlaps ${floorItems[j].definitionId}`, [floorItems[i].instanceId, floorItems[j].instanceId]));
                    }
                }
            }
            return out;
    ...
    },
    {
        ...
        evaluate: ({ design, brief }) => {
            ...
            return design.runWalls
                ...
                .map(wall => finding('allowed-wall', 'hard', `The design places cabinets on wall ${wall}, which was not selected for cabinetry`));
        },
    },
    {
        ...
        evaluate: ({ design }) => {
            ...
            return [...duplicates].map(wall => finding('duplicate-run-wall', 'hard', `Wall ${wall} is defined more than once`));
        },
    },
    {
        ...
        evaluate: ({ floorItems, room }) => floorItems.flatMap(item => {
            if (item.definitionId.includes('corner'))
                return [];
            const r = (0, geometry_1.itemRect)(item);
            const touching = [];
            if (r.minZ <= 30)
                touching.push('N');
            if (r.maxX >= room.width - 30)
                touching.push('E');
            if (r.maxZ >= room.depth - 30)
                touching.push('S');
            if (r.minX <= 30)
                touching.push('W');
            ...
            return touching.some(w => rot === geometry_1.WALL_ROTATION[w])
                ? []
                : [finding('faces-wall', 'hard', `${item.definitionId} touches wall(s) ${touching.join('/')} with rotation ${rot} — it faces a wall, not the room`, [item.instanceId])];
        }),
    },
    {
        ...
        evaluate: ({ floorItems, room }) => {
            ...
            for (const o of room.openings) {
                ...
                for (const item of floorItems) {
                    ...
                    if (o.wall === 'N') {
                        againstWall = r.minZ <= 30;
                        t = { start: r.minX, end: r.maxX };
                    }
                    if (o.wall === 'S') {
                        againstWall = r.maxZ >= room.depth - 30;
                        t = { start: room.width - r.maxX, end: room.width - r.minX };
                    }
                    if (o.wall === 'E') {
                        againstWall = r.maxX >= room.width - 30;
                        t = { start: r.minZ, end: r.maxZ };
                    }
                    if (o.wall === 'W') {
                        againstWall = r.minX <= 30;
                        t = { start: room.depth - r.maxZ, end: room.depth - r.minZ };
                    }
                    if (!againstWall || !t)
                        continue;
                    const gap = Math.max(span.start - t.end, t.start - span.end);
                    if (gap > -1 && gap < 25 && !item.fillerLeft && !item.fillerRight) {
                        out.push(finding('doorway-tight', 'safety', `${item.definitionId} is ${Math.max(0, Math.round(gap))}mm from the ${o.type} on wall ${o.wall} with no filler`, [item.instanceId]));
                    }
                }
            }
            return out;
    ...
    },
    {
        ...
        evaluate: ({ floorItems, room }) => {
            const out = [];
            for (const o of room.openings) {
                if (o.type !== 'door' || o.swing === 'out' || o.swing === 'slider')
                    continue;
                const c = (0, geometry_1.wallPointWorld)(o.wall, o.offsetMm + o.widthMm / 2, room);
                const half = o.widthMm / 2;
                const arc = { minX: c.x - half, maxX: c.x + half, minZ: c.z - half, maxZ: c.z + half };
                for (const item of floorItems) {
                    if ((0, geometry_1.rectsOverlap)((0, geometry_1.itemRect)(item), arc)) {
        ...
        evaluate: ({ floorItems, islandItems }) => {
            ...
            for (const isl of islandItems) {
                for (const other of floorItems) {
                    if (islandItems.includes(other))
                        continue;
                    const a = (0, geometry_1.itemRect)(isl), b = (0, geometry_1.itemRect)(other);
                    if (a.minX < b.maxX && a.maxX > b.minX) {
                        const gap = Math.max(b.minZ - a.maxZ, a.minZ - b.maxZ);
                        if (gap > 0 && gap < MIN_AISLE) {
                            out.push(finding('narrow-aisle', 'hard', `Aisle ${Math.round(gap)}mm between island and run (min ${MIN_AISLE}mm)`, [isl.instanceId, other.instanceId]));
                        }
                    }
                }
            }
    ...
    },
    {
        ...
        evaluate: ({ floorItems, islandItems }) => {
            ...
            if (!nRun.length || !sRun.length)
                return [];
            const gap = Math.min(...sRun.map(i => (0, geometry_1.itemRect)(i).minZ)) - Math.max(...nRun.map(i => (0, geometry_1.itemRect)(i).maxZ));
    ...
    },
    {
        ...
        evaluate: ({ design, brief }) => brief?.appliances.dishwasher && !design.rolePositions.dishwasher
    ...
    },
    {
        ...
        evaluate: ({ design, brief }) => {
            ...
            if (!brief?.appliances.dishwasher || !sink || !dishwasher)
                return [];
            const adjacent = sink.wall === dishwasher.wall && (Math.abs((sink.startMm + sink.widthMm) - dishwasher.startMm) <= 1
                || Math.abs((dishwasher.startMm + dishwasher.widthMm) - sink.startMm) <= 1);
            return adjacent ? [] : [finding('dishwasher-not-adjacent', 'hard', 'Dishwasher must be immediately beside the sink cabinet', [sink.item.instanceId, dishwasher.item.instanceId])];
        },
    },
    {
        ...
        evaluate: ({ floorItems }) => floorItems.flatMap(item => item.definitionId.includes('corner') && !item.blindSide
    ...
    },
    {
        ...
        evaluate: ({ design, brief }) => {
            ...
            if (fridge && fridge.widthMm + 20 < nominated) {
                out.push(finding('appliance-gap-fit', 'safety', `Fridge space is ${fridge.widthMm}mm but the nominated fridge needs ${nominated}mm`, [fridge.item.instanceId]));
            }
            const dw = design.rolePositions.dishwasher;
            if (dw && dw.widthMm < 600) {
                out.push(finding('appliance-gap-fit', 'safety', `Dishwasher opening is ${dw.widthMm}mm (a standard dishwasher needs 600mm)`, [dw.item.instanceId]));
            }
            return out;
        },
    },
    {
        ...
        evaluate: ({ design, room }) => {
            ...
            if (!sink || !drain)
                return [];
            const d = (0, geometry_1.dist)({ x: sink.item.x, z: sink.item.z }, (0, geometry_1.wallPointWorld)(drain.wall, drain.offsetMm, room));
    ...
    },
    {
        ...
        evaluate: ({ design, room, brief }) => {
            ...
            if (!cooktop || brief?.appliances.cooktop !== 'gas' || !gas)
                return [];
            const d = (0, geometry_1.dist)({ x: cooktop.item.x, z: cooktop.item.z }, (0, geometry_1.wallPointWorld)(gas.wall, gas.offsetMm, room));
            return d > GAS_MAX
    ...
    },
    {
        ...
        evaluate: ({ design, floorItems }) => {
            ...
            const cr = (0, geometry_1.itemRect)(cooktop.item);
            const hasSide = (side) => sameWall.some(i => {
                const r = (0, geometry_1.itemRect)(i);
    ...
    },
    {
        ...
        evaluate: ({ design }) => {
            ...
            const perimeter = (0, geometry_1.dist)(pts[0], pts[1]) + (0, geometry_1.dist)(pts[1], pts[2]) + (0, geometry_1.dist)(pts[2], pts[0]);
    ...
    },
    {
        ...
        evaluate: ({ design }) => {
            ...
            if (!sink || !cooktop || !fridge)
                return [];
            const pts = [sink.item, cooktop.item, fridge.item].map(i => ({ x: i.x, z: i.z }));
            const legs = [(0, geometry_1.dist)(pts[0], pts[1]), (0, geometry_1.dist)(pts[1], pts[2]), (0, geometry_1.dist)(pts[2], pts[0])];
            for (const leg of legs) {
                if (leg < LEG_MIN)
                    return [finding('triangle-leg', 'soft', 'Two work zones are very close together')];
                if (leg > LEG_MAX)
                    return [finding('triangle-leg', 'soft', 'Two work zones are far apart — expect extra walking')];
            }
            return [];
        },
    },
...
exports.RESERVED_RULE_IDS = ['sink-bowl-fit', 'run-end-panel', 'filler-complete', 'cutout-intersection'];
/** Build the shared context once, then run every rule. Deterministic order. */
function evaluateRules(design, room, brief) {
    ...
    const ctx = { design, room, brief, floorItems, islandItems };
    return exports.RULES.flatMap(rule => rule.evaluate(ctx));
}
...
exports.RULE_INDEX = exports.RULES.map(({ id, tier, scope, title }) => ({ id, tier, scope, title }));
/** Plain-language "why" for a rule id — used by the design-explanation feature. */
function ruleWhy(ruleId) {
    return exports.RULES.find(r => r.id === ruleId)?.why;
}
