...
exports.rotationFromNormal = rotationFromNormal;
exports.polygonFromRoom = polygonFromRoom;
exports.segmentToWorld = segmentToWorld;
exports.pointInPolygon = pointInPolygon;
exports.distToBoundary = distToBoundary;
exports.pointInsideOrNear = pointInsideOrNear;
exports.rectInsidePolygon = rectInsidePolygon;
exports.segmentForWall = segmentForWall;
exports.segmentSharedCornerAt = segmentSharedCornerAt;
exports.interiorAngles = interiorAngles;
const EPS = 1e-6;
function sub(a, b) { return { x: a.x - b.x, z: a.z - b.z }; }
function len(v) { return Math.hypot(v.x, v.z); }
function norm(v) { const l = len(v) || 1; return { x: v.x / l, z: v.z / l }; }
...
function makeSegment(a, b, id, legacyWall) {
...
function polygonFromRoom(room) {
    ...
    const corner = room.cutoutCorner ?? 'SE';
    ...
    switch (corner) {
        case 'SE': {
            const r1 = { x: W, z: D - cd }; // E wall foot (shortened)
            const reflex = { x: cw, z: D - cd };
            const r2 = { x: cw, z: D }; // S wall end (shortened)
        ...
        case 'SW': {
            const r1 = { x: W - cw, z: D }; // S wall end (shortened from left)
            const reflex = { x: W - cw, z: D - cd };
            const r2 = { x: 0, z: D - cd }; // W wall foot (shortened)
        ...
        case 'NE': {
            const r1 = { x: W - cw, z: 0 }; // N wall end (shortened from right)
            const reflex = { x: W - cw, z: cd };
            const r2 = { x: W, z: cd }; // E wall head (shortened)
        ...
        case 'NW': {
            const r1 = { x: cw, z: 0 }; // N wall start (shortened from left)
            const reflex = { x: cw, z: cd };
            const r2 = { x: 0, z: cd }; // W wall head (shortened)
...
function segmentToWorld(seg, t, widthMm, depthMm) {
...
function rectInsidePolygon(rect, poly, tol = 1) {
    ...
    if (rect.maxX - rect.minX > 2 * inset && rect.maxZ - rect.minZ > 2 * inset) {
        ...
        for (let i = 0; i < ic.length; i++) {
            ...
            for (const s of poly.segments) {
                if (segmentsCross(c1, c2, s.a, s.b))
                    return false;
            }
        }
    }
...
function segmentForWall(poly, wall) {
 ...
 */
function segmentSharedCornerAt(segA, segB, tol = 1) {
    const near = (p, q) => len(sub(p, q)) <= tol;
    if (near(segA.a, segB.a) || near(segA.a, segB.b))
        return 'start';
    if (near(segA.b, segB.a) || near(segA.b, segB.b))
        return 'end';
    return null;
}
...
function interiorAngles(poly) {
    const v = poly.vertices, n = v.length, out = [];
