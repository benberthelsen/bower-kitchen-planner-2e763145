...
function strategyPlausible(strategy, brief) {
    ...
    const ok = (...walls) => !allowed || walls.every(w => allowed.includes(w));
    const okAny = (...walls) => !allowed || walls.some(w => allowed.includes(w));
    switch (strategy) {
...
function mirrorSideRuns(spec, allowedWalls) {
    const flip = { W: 'E', E: 'W' };
