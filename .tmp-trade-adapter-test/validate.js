...
function validate(design, room, brief) {
    ...
    if (sink && drain) {
        ...
        const drainPt = (0, geometry_1.wallPointWorld)(drain.wall, drain.offsetMm, room);
        const d = (0, geometry_1.dist)(sinkPt, drainPt);
        if (d > SINK_DRAIN_MAX) {
    ...
    // ── essentials present ──
    if (!sink)
        v.push({ code: 'no-sink', severity: 'error', message: 'Design has no sink cabinet' });
    if (!cooktop)
        v.push({ code: 'no-cooktop', severity: 'error', message: 'Design has no cooktop cabinet' });
    const fridge = rolePositions['fridge-gap'];
    if (!fridge)
        v.push({ code: 'no-fridge', severity: 'error', message: 'Design has no fridge space' });
    ...
    if (brief?.appliances.dishwasher && sink && dishwasher) {
        const adjacentOnSameWall = sink.wall === dishwasher.wall && (Math.abs((sink.startMm + sink.widthMm) - dishwasher.startMm) <= 1
    ...
    if (sink && cooktop && fridge) {
        ...
        for (const leg of legs) {
            if (leg < LEG_MIN) {
                v.push({ code: 'triangle-leg', severity: 'warn', message: 'Two work zones are very close together' });
                break;
            }
            if (leg > LEG_MAX) {
                v.push({ code: 'triangle-leg', severity: 'warn', message: 'Two work zones are far apart — expect extra walking' });
                break;
            }
        }
    }
