...
import type { DesignBrief, KitchenSpec, SegmentRole, Violation } from './types';
import { ROLE_PRODUCTS } from './catalogRoles';

export interface DesignScoreParts {
  appliances: number;        // required appliances represented (0-25)
  priorities: number;        // brief priorities satisfied (0-20)
  workflow: number;          // sink/cooktop/dishwasher relationships (0-25)
  serviceProximity: number;  // plumbing/gas moves avoided (0-15)
  benchContinuity: number;   // longest uninterrupted base-cabinet stretch (0-15)
  warningsPenalty: number;   // residual warnings (0 to -15)
}
...
function rolesOf(spec: KitchenSpec): SegmentRole[] {
  return spec.runs.flatMap(run =>
    run.segments.flatMap(s => (s.kind === 'cabinet' ? [s.role] : [])));
}

function defaultWidth(role: SegmentRole, widthMm?: number): number {
  return widthMm ?? ROLE_PRODUCTS[role]?.widths[0] ?? 600;
}
...
function longestBenchStretch(spec: KitchenSpec): number {
  ...
  for (const run of spec.runs) {
    ...
    for (const s of run.segments) {
      ...
      if (isBench) {
        current += defaultWidth((s as { role: SegmentRole; widthMm?: number }).role, (s as { widthMm?: number }).widthMm);
...
export function scoreDesign(
  ...
  const has = (role: SegmentRole) => roles.includes(role);
  const count = (role: SegmentRole) => roles.filter(r => r === role).length;
  const warns = violations.filter(v => v.severity === 'warn');
  const warnCodes = new Set(warns.map(w => w.code));

  // Required appliances represented (0-25)
  let appliances = 0;
  if (has('sink')) appliances += 8;
  if (has('cooktop')) appliances += 7;
  if (has('fridge-gap')) appliances += 5;
  if (brief.appliances.dishwasher) appliances += has('dishwasher') ? 5 : 0;
  else appliances += 5; // nothing missing
  ...
  if (sinkRun >= 0 && cookRun >= 0) workflow += Math.abs(sinkRun - cookRun) <= 1 ? 8 : 3;
  if (!warnCodes.has('triangle-size') && !warnCodes.has('triangle-leg')) workflow += 7;
  workflow = Math.min(25, workflow);

  // Service proximity (0-15): validate() already measured the distances.
  let serviceProximity = 15;
  if (warnCodes.has('replumb')) serviceProximity -= 8;
  if (warnCodes.has('gas-move')) serviceProximity -= 7;
  ...
  const parts: DesignScoreParts = {
    appliances, priorities, workflow, serviceProximity, benchContinuity, warningsPenalty,
  };
  const total = Math.max(0, Math.min(100,
    Math.round(appliances + priorities + workflow + serviceProximity + benchContinuity + warningsPenalty)));
  return { total, parts };
}
