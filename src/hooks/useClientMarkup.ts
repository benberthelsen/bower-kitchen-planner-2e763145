    ...
    queryFn: async () => {
      ...
      if (!rows || rows.length === 0) {
        const { data: anyRow } = await supabase
          ...
          .eq('client_id', user!.id)
          .limit(1);
        if (anyRow?.[0]) return anyRow[0];
        // Global default (client_id NULL, is_default) — seeded by the
        // 20260708 migration so quotes always carry a commercial layer.
