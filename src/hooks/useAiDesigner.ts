 ...
 * const res = await refine(brief, shape, spec, 'move the sink under the window');
 * const res = await restyle(brief, shape, spec, 'warm coastal, oak + white');
 *
...
export interface AiDesignResult {
  ...
  unchanged?: boolean;
  room?: DesignBrief['room']; // may be patched by refine turns ("there's a door on the east wall")
}
...
export function useAiDesigner() {
  ...
  const refine = useCallback(
    (brief: DesignBrief, shape: LayoutShape, currentSpec: KitchenSpec, message: string, history: ChatTurn[] = []) =>
      call({ mode: 'refine', brief, shape, currentSpec, message, history }),
    [call],
  );

  const restyle = useCallback(
    (brief: DesignBrief, shape: LayoutShape, currentSpec: KitchenSpec, message: string) =>
      call({ mode: 'style', brief, shape, currentSpec, message }),
    [call],
  );
