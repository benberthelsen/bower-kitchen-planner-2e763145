...
function inferStaticMetadata(item: StaticCatalogTemplate) {
  ...
  // Shared with the pricing engine so the cabinet DRAWS with the same fronts
  // it is CHARGED for. The previous local version matched only 1_door/2_door/
  // 3_door and fell through to `return 2`, so fillers, toe kicks, light rails
  // and applied panels rendered as two-door cabinets; drawers capped at 4, so
  // a 5/6/7-drawer bank drew with none.
  ...
  const doorCount = (() => {
    ...
    if (nameLower.includes('open shelf') || nameLower.includes('open unit')
        || (nameLower.match(/\bopen\b/) && !nameLower.includes('opening'))) return 0;
    ...
    if (dims.category === 'Wall' || dims.category === 'Tall') return 2;
    return 2;
  })();
