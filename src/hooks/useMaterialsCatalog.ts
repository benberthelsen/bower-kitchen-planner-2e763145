...
export interface HardwareOptionRow {
  ...
  handlingCost: number | null;
  /** Supplier product photo (from the local feed, keyed by item_code). */
...
export function useMaterialsCatalog() {
  ...
  const hardwareQuery = useQuery({
    ...
    queryFn: async (): Promise<HardwareOptionRow[]> => {
      // Product photos live in the local supplier feed keyed by item_code
      // (the Supabase table has no image column).
      let images: Record<string, string> = {};
      try {
        const res = await fetch('/data/bower-supplier-catalog/hardware-images.json');
  ...
  // Make catalog handles resolvable by the 3D scene (registry lookup — the
  // canvas components can't use react-query hooks across renderers).
  useEffect(() => {
    if (handles.length > 0) {
      registerHandleRows(handles.map((h) => ({
        id: h.id,
        name: h.name,
        brand: h.brand,
        series: h.series,
