...
const CabinetAssembler: React.FC<CabinetAssemblerProps> = ({
  ...
  // consistent; add a real per-material repeat to the supplier catalogue and
  ...
  // Toe kick: the ROOM's setting wins, recipe is the fallback. Reversed, a room
  // configured with a 150mm kick still drew 135 because the recipe supplied one.
      ...
      return (
        <React.Fragment key={`drawer-${i}`}>
          <DrawerFront
            ...
            showBox={true}
            forceOpen={doorsOpen}
            // Handle now lives INSIDE DrawerFront so it slides with the drawer.
            handle={{ type: handle.type, color: handle.hex }}
          />
        </React.Fragment>
      );
