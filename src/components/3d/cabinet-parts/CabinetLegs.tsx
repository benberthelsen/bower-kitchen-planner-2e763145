...
const CabinetLegs: React.FC<CabinetLegsProps> = ({
  ...
  const getPositions = (): [number, number, number][] => {
    ...
    // Front legs sit BEHIND the kickboard face (which is recessed ~40mm from the
    // front). Use a deeper front setback so the leg + its wider foot stay hidden
    // behind the kick panel instead of poking through it.
    const frontSetback = Math.max(setback, 0.09);

    const positions: [number, number, number][] = [
      // Front left
      [-width / 2 + setback, yBase, depth / 2 - frontSetback],
      // Front right
      [width / 2 - setback, yBase, depth / 2 - frontSetback],
      // Back left
    ...
    if (hasMiddleLeg) {
      positions.push(
        [0, yBase, depth / 2 - frontSetback], // Front middle
