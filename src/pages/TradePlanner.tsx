...
function TradePlannerContent() {
  ...
  const handleSaveDesign = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('jobs').insert([{
        name: projectSettings.jobName || 'New Trade Design',
        customer_id: user.id,
  ...
  return (
    <div className="flex h-screen w-screen bg-gray-100 font-sans text-gray-900 flex-col md:flex-row overflow-hidden relative">
      <header className="hidden md:flex absolute top-0 left-0 right-0 h-14 items-center justify-between px-4 bg-gray-900 text-white border-b border-gray-700 z-30">
        <div className="flex items-center gap-3">
          <div className="font-bold text-lg">Bower</div>
          <Badge variant="secondary" className="bg-amber-500 text-black hover:bg-amber-400">
        ...
        <div className="flex items-center gap-2">
          <button className="px-3 py-1.5 rounded border border-gray-600 hover:bg-gray-800 flex items-center gap-2 disabled:opacity-50" onClick={undo} disabled={!canUndo} title="Undo (Ctrl+Z)">
            <Undo2 size={16} />
          </button>
          <button className="px-3 py-1.5 rounded border border-gray-600 hover:bg-gray-800 flex items-center gap-2 disabled:opacity-50" onClick={redo} disabled={!canRedo} title="Redo (Ctrl+Shift+Z)">
            <Redo2 size={16} />
          </button>

          <div className="h-6 w-px bg-gray-600 mx-1" />

          <button className={`px-3 py-1.5 rounded border border-gray-600 hover:bg-gray-800 flex items-center gap-2 ${!is3D ? "bg-gray-700" : ""}`} onClick={() => setIs3D(false)}>
            <Grid3X3 size={16} />
            2D
          </button>
          <button className={`px-3 py-1.5 rounded border border-gray-600 hover:bg-gray-800 flex items-center gap-2 ${is3D ? "bg-gray-700" : ""}`} onClick={() => setIs3D(true)}>
            <Box size={16} />
            3D
          </button>
          ...
          <button className="px-3 py-1.5 rounded border border-gray-600 hover:bg-gray-800 flex items-center gap-2" title="Keyboard shortcuts">
            <HelpCircle size={16} />
          </button>
        </div>
      </header>
