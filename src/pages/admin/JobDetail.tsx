  ...
  const quoteBOM = useMemo(() => {
    ...
    const placedItems = allCabinets
      ...
      .map((c) => ({
        ...
        edgeId: (c.materials as Record<string, string> | undefined)?.edgeBanding,
      }));
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {cabs.map((cab, ci) => {
                                ...
                                const instId = cab.instanceId as string;
                                // Prefer the persisted SELL price. For older snapshots that only
                                // stored raw cost in perCabinetTotals, scale it to sell by the
                                // room's cost→sell ratio so admin never shows bare cost as "Sell".
                                const sellMap = snap?.perCabinetSell as Record<string, number> | undefined;
                                const costMap = snap?.perCabinetTotals as Record<string, number> | undefined;
                                let perCab = sellMap?.[instId];
                                if (perCab == null && costMap) {
                                  const cost = costMap[instId];
                                  const grand = (snap?.bomSummary as { grandTotal?: { total?: number } } | undefined)?.grandTotal?.total
                                    ?? (snap?.roomTotal as number | undefined);
                                  const costSum = Object.values(costMap).reduce((s, v) => s + (v || 0), 0);
                                  const factor = grand && costSum > 0 ? grand / costSum : 1;
                                  perCab = cost != null ? Math.round(cost * factor * 100) / 100 : undefined;
                                }
                                return (
