...
export default function AdminReports() {
  ...
  async function loadAll() {
    setLoading(true);
    // allSettled + finally: one failing query (or a bad row) must never leave
    // the page stuck on the loading spinner (the "blank reports page").
    try {
      await Promise.allSettled([loadRevenue(), loadStatusBreakdown(), loadFunnel()]);
    } catch (err) {
  ...
  async function loadRevenue() {
    ...
    data.forEach(j => {
      ...
      if (!d || Number.isNaN(d.getTime())) return; // skip rows with bad/missing dates
      const key = format(d, 'MMM yyyy');
      if (buckets[key]) {
