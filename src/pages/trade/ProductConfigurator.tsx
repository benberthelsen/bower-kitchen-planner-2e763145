...
export default function ProductConfigurator() {
  ...
  const { upsertCabinet, jobQuery, roomsFromServer } = useTradeJobPersistence(jobId);

  // Deep-link / hard-reload of /configure/...?edit= lands with an empty
  // in-memory room context (the planner never ran), so getRoomById returned
  // nothing and the editor hung on "Loading cabinet" (review #4). Hydrate from
  // the persisted job first, exactly like RoomPlanner/JobEditor do.
  useEffect(() => {
    if (jobId && jobId !== 'new' && jobQuery.data && rooms.length === 0 && roomsFromServer.length > 0) {
      hydrateRooms(roomsFromServer);
    }
  }, [jobId, jobQuery.data, rooms.length, roomsFromServer, hydrateRooms]);
  ...
  useEffect(() => {
    ...
    setCurrentRoom(room);
    // `rooms` in deps so the room re-resolves once server hydration lands
    // (otherwise a deep-link stays on the null room).
  }, [roomId, rooms, getRoomById, setCurrentRoom]);
